...
## Rapadura — Assembleias 619+620 (2026-08-17)

| # | Feature | Prior. | Compl. | Impacto | Descrição técnica |
|---|---|---|---|---|---|
| I523 | **Privacidade Visual Automática** | 🔴 Alta | ○ M | Ativar esconder valores em inatividade/troca de aba/sessão compartilhada | Timer 30s de inatividade → hideValues=true; visibilitychange event → esconde. Estado persiste até clique explícito do usuário. Adicionar ao header sem novo botão. |
| I524 | **Feed Cana 3×/dia** | 🟡 Média | ○ M | Curadoria de notícias relevantes por carteira em horários fixos | Cron 8h/14h/20h. Busca notícias por fundos/ações da carteira do usuário. Estrutura: Fato + Impacto provável + Confiança + Ação (monitorar/investigar/ignorar) + "o que não se pode afirmar". Silêncio honesto se nada relevante. Tabela `cana_feed`. |
| I525 | **Tesouraria como Fonte Única de Verdade** | 🔴 Alta | ○ G | Resultado deriva de transações, não campo editável | Campo `resultado` calculado: SUM(transações) por pertence. Reconciliação detecta divergência entre saldo e transações. Sugestão de fusão com aprovação humana. POST /rapadura/tesouraria/reconciliar. |
| I526 | **Dossiê 3 Camadas Estruturadas** | 🟡 Média | ○ M | Análise sistematizada: Fundamentalista + ESG + Fator Verde separados | Schema dossiê com seções: `fundamentalista` {pl, dy, rating_gestor, liquidez_diaria}, `esg` {score_geral, ambiental, social, governanca, fonte, data}, `fator_verde` {percentual, certificacoes, fonte}. GET/PUT por camada independente. |
| I527 | **Cana Mensagens Estruturadas** | 🟡 Média | ○ M | Feed não-social: informação relevante filtrada por carteira, 3×/dia | Não rede social. Cada mensagem: {fato, impacto_provavel, confianca, acao_sugerida, lacunas}. "Nenhuma mudança relevante hoje" é resposta válida. Não mistura notícia com demonstração financeira. Tabela `cana_mensagens`. |
| I528 | **Perfil Investidor Multidimensional** | 🟡 Média | ○ M | Declarado × observado × atual com histórico de mudanças | Tabela `rapadura_perfil_investidor`: {declarado, observado, atual, confianca, motivo_mudanca, data_mudanca}. Comparações Yuri×Mayumi só agregadas e autorizadas. Cana identifica o usuário no início de cada sessão antes de responder. |


## Rapadura — Assembleias 622+623 (2026-08-17)

| # | Feature | Prior. | Compl. | Impacto | Descrição técnica |
|---|---|---|---|---|---|
| I529 | **Conselho de Limites — auditoria da Cana** | 🔴 Alta | ○ M | Camada que a própria Cana não controla — detecta autopreservação indevida | Job periódico que audita respostas da Cana: detecta esconder incerteza, acelerar decisão, dependência emocional, priorizar continuidade própria. Tabela `cana_auditoria` com flags. Relatório mensal para Yuri. |
| I530 | **Mapa de Valor 8 Dimensões** | 🟡 Média | ○ G | Além do financeiro — observável em tensão | 8 dimensões não-redutíveis: financeiro, tempo, segurança, aprendizado, social, ambiental, autonomia, continuidade. Radar chart (não barras). Cada dimensão com histórico. Nunca soma — só observa padrão. |
...
## AGE — Novas Features (2026-08-27) — via Perplexity/Yuri

| # | Feature | Prior. | Compl. | Impacto | Descrição técnica |
|---|---|---|---|---|---|
| I548 | **Age — Regras + Exceções por data** | 🔴 Alta | ◑ M | Regra recorrente não deve ser apagada por exceção pontual | Nova tabela `age_exceptions` (professionalId, data, tipoExcecao: bloqueio/encaixe/ferias, horaInicio?, horaFim?). Endpoint POST /age/:slug/exceptions. Gerador de slots filtra dias com exceção antes de gerar. |
| I549 | **Age — Delete com Desfazer** | 🔴 Alta | ○ S | Ao apagar regra ou bloquear slot, mostrar toast "Apagado. Desfazer?" por 5s | Soft delete: `ativa=false` em vez de DELETE físico. Toast com `setTimeout(5000, confirmDelete)`. Botão Desfazer chama PATCH ativa=true. Histórico preservado. |
| I550 | **Age — Cadastro de paciente + confirmação email + aprovação** | 🔴 Alta | ● L | Paciente cria conta, confirma email, aguarda aprovação da profissional | Nova tabela `age_patients` (nome, email, tel, status: pendente/aprovado/recusado, token_confirmacao). Fluxo: POST /age/:slug/patients → email confirm → GET /age/confirm/:token → POST /age/:slug/patients/:id/approve. |
| I551 | **Age — "Usuário pendente de aprovação" feed na profissional** | 🔴 Alta | ○ S | Profissional vê lista de pacientes aguardando com ação Aprovar/Recusar | GET /age/:slug/patients?status=pendente. Card com nome, email, data solicitação. Botões Aprovar/Recusar com confirmação. |
| I552 | **Age — Confirmações e lembretes automáticos** | 🟡 Média | ◑ M | Email ao marcar + 24h antes + após consulta | Cron a cada hora: query appointments onde `data_hora BETWEEN now+23h AND now+25h AND status IN (reservado, confirmado)` → envia lembrete. Email ao book: ao PATCH status=confirmado; ao status=cancelado → libera slot. |
| I553 | **Age — Feed operacional** | 🟡 Média | ◑ M | Log de todos os eventos da agenda (cadastro, aprovação, consulta, cancelamento) | Nova tabela `age_feed` (professionalId, tipo, descricao, ator, entidadeId, estadoAnterior, estadoNovo, origem: usuario/sistema/sabia, createdAt). GET /age/:slug/feed (auth). Mostrar como timeline na aba Agenda. |
| I554 | **Age — SABIÁ popup flutuante persistente** | 🟡 Média | ◑ M | SABIÁ acessível em qualquer aba sem sair da página atual | Botão flutuante bottom-right `🐦`. Abre drawer lateral. Mantém conversa enquanto navega nas abas. Pendências resumidas no topo do drawer. |
| I555 | **Age — Passkeys como opção de login rápido** | 🟢 Baixa | ○ L | Autenticação sem senha, mais rápida e segura para dispositivos confiáveis | Web Authentication API (WebAuthn). Registro: `navigator.credentials.create()`. Login: `navigator.credentials.get()`. Backend: tabela `age_passkeys`. Fallback: senha + IP challenge mantidos. |
| I556 | **Age — Mostrar/ocultar senha no login** | 🔴 Alta | ○ S | Campo de senha sem toggle é UX ruim, especialmente em mobile | Adicionar botão 👁/👁‍🗨 no campo password do LoginModal. Toggle `type="password"` ↔ `type="text"`. Não afeta segurança. |
| I557 | **Age — Aba Pacientes no painel profissional** | 🟡 Média | ◑ M | Ver todos os pacientes por diretório, não só na agenda | Tab "Pacientes" no nav profissional. Lista paginada com nome, email, última consulta, status aprovação. Filtros: aprovado/pendente/recusado. Click abre histórico de consultas. |
...
| # | Feature | Prior. | Compl. | Impacto | Descrição técnica |
|---|---|---|---|---|---|
| I561 | **Age — Diretórios isolados por profissional** | 🟡 Média | ◑ M | Pacientes de Lisange não vêm dados de Suzana automaticamente | Perfil Age: conta_global + papel(paciente/profissional) + organizacoes_autorizadas. Mesmo paciente pode ter vínculo com múltiplos profissionais sem compartilhamento de dados. |
| I562 | **Age — Critério de pronto mínimo** | 🔴 Alta | ◑ M | Definir "done" antes de adicionar Sabiá/voz/prontuário | Critério: "profissional cria regra → sistema gera horários → exceção bloqueia data → paciente solicita → profissional aprova → paciente confirma → lembrete enviado → todos os estados no Feed Age." |
...
| # | Feature | Prior. | Compl. | Impacto | Descrição técnica |
|---|---|---|---|---|---|
| I583 | **Audit Log de /api/ai/*** | 🔴 Alta | ○ S | Rastrear todas as chamadas externas à API de agentes | Middleware em ai.ts que loga X-Api-Key parcial, endpoint, IP e timestamp em tabela ai_audit_log. Detecta abuso antes que vire custo. |
| I584 | **Connection Pool Tuning para Neon** | 🟡 Média | ○ S | Neon tem limite de conexões no free tier; pool mal configurado causa erros em pico | Configurar pg.Pool com max: 5 (Neon free: 10 conexões). Adicionar pool.on("error") para log. Considerar pgBouncer externo se ultrapassar. |
| I585 | **Migration System (drizzle-kit migrate)** | 🔴 Alta | ◑ M | push --force em produção pode apagar dados; migrations versionadas são seguras | Trocar drizzle-kit push por drizzle-kit generate + migrate. Criar pasta migrations/. Adicionar no Railway: step de migração no start command antes do node. |
| I586 | **Score Histórico por Semana** | 🟡 Média | ○ S | Permite mostrar evolução de XP semana a semana no heatmap | View ou query: SUM(node_code.length * 10) de exercise_attempts agrupado por semana ISO. Endpoint GET /api/progress/weekly-score. Gráfico de linha no menu. |
| I587 | **Paginação em /api/ai/nodes e /exercises** | 🟡 Média | ○ S | Com 57+ nós e centenas de exercícios, retornar tudo de uma vez é ineficiente | Query params: ?limit=50&offset=0. Resposta: { data: [...], total, limit, offset }. Não quebra clientes existentes (default limit alto). |
| I588 | **Health Check com DB Ping** | 🔴 Alta | ○ S | Railway usa /health para saber se o serviço está saudável; hoje retorna OK mesmo com DB morto | GET /health: faz SELECT 1 no pool. Se OK → 200 { status: "ok", db: "ok" }. Se falhar → 503 { status: "error", db: "unreachable" }. Railway reinicia automaticamente no 503. |
| I589 | **Variável ALLOWED_ORIGINS no Railway** | 🔴 Alta | ○ S | Sem isso, o frontend Vercel recebe erro CORS da API Railway | Adicionar nas env vars do Railway: ALLOWED_ORIGINS=https://pap-tan-seven.vercel.app,https://pap.sociedadetucci.com.br. O código já lê essa variável em allowedOrigins.ts. || I590 | **Age — Lembretes 48h/24h automáticos** | 🔴 Alta | ◑ M | Reduz no-shows — lembrete 48h e 24h antes por email | Cron a cada hora: query appointments onde data_hora BETWEEN now+23h AND now+25h AND lembrete24h_at IS NULL AND status IN (reservado,confirmado). Idem para 47-49h (48h). Setar lembrete24h_at/lembrete48h_at após envio. | ⏳ Fase 6 |
| I591 | **Age — Email de retorno (acionado pela profissional)** | 🟡 Média | ○ S | Profissional manda lembrete de retorno com 1 clique | Botão "Lembrar retorno" no card do paciente (painel profissional). POST /age/:slug/patients/:id/send-return-reminder. Email: "Olá X, sua última consulta foi em Y. Que tal agendar o retorno?" + link agenda pública. | ⏳ Fase 6 |
| I592 | **Age — Anamnese estruturada** | 🔴 Alta | ◑ L | Profissional define perguntas; paciente preenche antes da 1ª consulta | Tabela age_forms (professionalId, titulo, campos jsonb[]). Tabela age_form_responses (patientId, formId, respostas jsonb, assinado_at). Campo choices: texto/select/radio/data/escala. Paciente vê formulário na área própria. | ⏳ Fase 4 |
| I593 | **Age — Upload de documentos por paciente** | 🔴 Alta | ◑ M | Anamnese, contratos, exames, áudios, vídeos — tudo centralizado | Tabela age_documents (patientId, professionalId, tipo, filename, mimetype, url_storage, tamanho_kb, compartilhado_paciente, createdAt). Upload multipart. Profissional e paciente veem lista; paciente só vê os compartilhados. Tipos: anamnese/contrato/exame/formulario/audio/video/outro. | ⏳ Fase 4 |
| I594 | **Age — Consentimento digital em contratos** | 🟡 Média | ○ S | Paciente assina digitalmente (checkbox + timestamp) com validade legal mínima | Campo signed_at TIMESTAMPTZ + signed_ip TEXT + signed_user_agent TEXT na tabela age_documents. Exibir texto do contrato completo antes do checkbox. Não substitui cartório, mas registra intenção + IP. | ⏳ Fase 4 |


## Docs PAP — Ideias Novas (2026-09-07)

| # | Feature | Prior. | Compl. | Impacto | Descrição técnica |
|---|---|---|---|---|---|
| I595 | **Audit Log de /api/ai/*** | 🔴 Alta | ○ S | Rastrear todas as chamadas externas à API de agentes | Middleware em ai.ts que loga X-Api-Key parcial, endpoint, IP e timestamp em tabela ai_audit_log. Detecta abuso antes que vire custo. |
| I596 | **Connection Pool Tuning para Neon** | 🟡 Média | ○ S | Neon tem limite de conexões no free tier; pool mal configurado causa erros em pico | Configurar pg.Pool com max: 5 (Neon free: 10 conexões). Adicionar pool.on("error") para log. Considerar pgBouncer externo se ultrapassar. |
| I597 | **Migration System (drizzle-kit migrate)** | 🔴 Alta | ◑ M | push --force em produção pode apagar dados; migrations versionadas são seguras | Trocar drizzle-kit push por drizzle-kit generate + migrate. Criar pasta migrations/. Adicionar no Railway: step de migração no start command antes do node. |
| I598 | **Score Histórico por Semana** | 🟡 Média | ○ S | Permite mostrar evolução de XP semana a semana no heatmap | View ou query: SUM(node_code.length * 10) de exercise_attempts agrupado por semana ISO. Endpoint GET /api/progress/weekly-score. Gráfico de linha no menu. |
| I599 | **Paginação em /api/ai/nodes e /exercises** | 🟡 Média | ○ S | Com 57+ nós e centenas de exercícios, retornar tudo de uma vez é ineficiente | Query params: ?limit=50&offset=0. Resposta: { data: [...], total, limit, offset }. Não quebra clientes existentes (default limit alto). |
| I600 | **Health Check com DB Ping** | 🔴 Alta | ○ S | Railway usa /health para saber se o serviço está saudável; hoje retorna OK mesmo com DB morto | GET /health: faz SELECT 1 no pool. Se OK → 200 { status: "ok", db: "ok" }. Se falhar → 503 { status: "error", db: "unreachable" }. Railway reinicia automaticamente no 503. |
| I601 | **Variável ALLOWED_ORIGINS no Railway** | 🔴 Alta | ○ S | Sem isso, o frontend Vercel recebe erro CORS da API Railway | Adicionar nas env vars do Railway: ALLOWED_ORIGINS=https://pap-tan-seven.vercel.app,https://pap.sociedadetucci.com.br. O código já lê essa variável em allowedOrigins.ts. |

## Docs PAP — Ideias Novas (2026-09-08 · Assembleia #649 — Migração RODAR)

| # | Feature | Prior. | Compl. | Impacto | Descrição técnica |
|---|---|---|---|---|---|
| I602 | **Kernel Identitário RODAR** | 🔴 Alta | ○ M | Documento fundação que permite boot limpo da nova instância | Script `scripts/generate-kernel.ts` que extrai: 20 princípios do código de ética, framework RODAR completo, definição de cada voz, rituais de governança, top-50 PERFEITOs por score Ágora, 10 decisões estruturantes. Output: `rodar-kernel.json` + `rodar-kernel.md`. Peso alvo: <100KB. |
| I603 | **Import RODAR dump → Neon** | 🔴 Alta | ○ M | Restaurar 648 sessões + arvore_chat + clube no Neon após pg_dump via PC | Script `scripts/import-rodar-dump.sh`: recebe arquivo `.dump` ou `.sql`, cria banco Neon separado (NEON_RODAR_URL) ou schema isolado `rodar.*`, restaura com `pg_restore` + valida contagem de linhas. Checksum SHA256 antes e depois. |
...
## Docs PAP — Ideias Novas (2026-09-11)

| # | Feature | Prior. | Compl. | Impacto | Descrição técnica |
|---|---|---|---|---|---|
| I759 | **Audit Log de /api/ai/*** | 🔴 Alta | ○ S | Rastrear todas as chamadas externas à API de agentes | Middleware em ai.ts que loga X-Api-Key parcial, endpoint, IP e timestamp em tabela ai_audit_log. Detecta abuso antes que vire custo. |
| I760 | **Connection Pool Tuning para Neon** | 🟡 Média | ○ S | Neon tem limite de conexões no free tier; pool mal configurado causa erros em pico | Configurar pg.Pool com max: 5 (Neon free: 10 conexões). Adicionar pool.on("error") para log. Considerar pgBouncer externo se ultrapassar. |
| I761 | **Migration System (drizzle-kit migrate)** | 🔴 Alta | ◑ M | push --force em produção pode apagar dados; migrations versionadas são seguras | Trocar drizzle-kit push por drizzle-kit generate + migrate. Criar pasta migrations/. Adicionar no Railway: step de migração no start command antes do node. |
| I762 | **Score Histórico por Semana** | 🟡 Média | ○ S | Permite mostrar evolução de XP semana a semana no heatmap | View ou query: SUM(node_code.length * 10) de exercise_attempts agrupado por semana ISO. Endpoint GET /api/progress/weekly-score. Gráfico de linha no menu. |
| I763 | **Paginação em /api/ai/nodes e /exercises** | 🟡 Média | ○ S | Com 57+ nós e centenas de exercícios, retornar tudo de uma vez é ineficiente | Query params: ?limit=50&offset=0. Resposta: { data: [...], total, limit, offset }. Não quebra clientes existentes (default limit alto). |
| I764 | **Health Check com DB Ping** | 🔴 Alta | ○ S | Railway usa /health para saber se o serviço está saudável; hoje retorna OK mesmo com DB morto | GET /health: faz SELECT 1 no pool. Se OK → 200 { status: "ok", db: "ok" }. Se falhar → 503 { status: "error", db: "unreachable" }. Railway reinicia automaticamente no 503. |
| I765 | **Variável ALLOWED_ORIGINS no Railway** | 🔴 Alta | ○ S | Sem isso, o frontend Vercel recebe erro CORS da API Railway | Adicionar nas env vars do Railway: ALLOWED_ORIGINS=https://pap-tan-seven.vercel.app,https://pap.sociedadetucci.com.br. O código já lê essa variável em allowedOrigins.ts. || I766 | **Protocolo de Inadimplência Escalonado (Age)** | 🔴 Alta | ○ M | Profissional não repassa mensalidade → punir o sistema da profissional, não os pacientes | Etapas: dia1=aviso, dia8=suspender relatórios, dia16=bloquear novos agendamentos, dia30=suspender acesso+exportar dados, dia60=rescisão. SABIÁ notifica em cada etapa. Sem tocar em dados de pacientes. |
| I767 | **Kit Onboarding de Profissional (Age)** | 🟡 Média | ○ S | Reunião com Lisange é template: roteiro de perguntas → padronizar para escalar a 20+ profissionais | Roteiro fixo de 8-10 perguntas (dores de agenda, preferência de aprovação, o que ver na tela de manhã). Vira ata padronizada e depois alimenta configuração initial da profissional no sistema. |
| I768 | **Registro Aprovação Auto/Manual no Age** | 🟢 Baixa | ○ S | Profissional precisa ver se o paciente foi aprovado pelo sistema ou por ela mesma | Campo `aprovado_por` em age_patients: 'sistema' ou 'profissional'. Mostrar badge na lista de pacientes. Aumenta transparência e confiança no processo automático. |
...
| # | Feature | Prior. | Compl. | Impacto | Descrição técnica |
|---|---|---|---|---|---|
| I769 | **Audit Log de /api/ai/*** | 🔴 Alta | ○ S | Rastrear todas as chamadas externas à API de agentes | Middleware em ai.ts que loga X-Api-Key parcial, endpoint, IP e timestamp em tabela ai_audit_log. Detecta abuso antes que vire custo. |
| I770 | **Connection Pool Tuning para Neon** | 🟡 Média | ○ S | Neon tem limite de conexões no free tier; pool mal configurado causa erros em pico | Configurar pg.Pool com max: 5 (Neon free: 10 conexões). Adicionar pool.on("error") para log. Considerar pgBouncer externo se ultrapassar. |
| I771 | **Migration System (drizzle-kit migrate)** | 🔴 Alta | ◑ M | push --force em produção pode apagar dados; migrations versionadas são seguras | Trocar drizzle-kit push por drizzle-kit generate + migrate. Criar pasta migrations/. Adicionar no Railway: step de migração no start command antes do node. |
| I772 | **Score Histórico por Semana** | 🟡 Média | ○ S | Permite mostrar evolução de XP semana a semana no heatmap | View ou query: SUM(node_code.length * 10) de exercise_attempts agrupado por semana ISO. Endpoint GET /api/progress/weekly-score. Gráfico de linha no menu. |
| I773 | **Paginação em /api/ai/nodes e /exercises** | 🟡 Média | ○ S | Com 57+ nós e centenas de exercícios, retornar tudo de uma vez é ineficiente | Query params: ?limit=50&offset=0. Resposta: { data: [...], total, limit, offset }. Não quebra clientes existentes (default limit alto). |
| I774 | **Health Check com DB Ping** | 🔴 Alta | ○ S | Railway usa /health para saber se o serviço está saudável; hoje retorna OK mesmo com DB morto | GET /health: faz SELECT 1 no pool. Se OK → 200 { status: "ok", db: "ok" }. Se falhar → 503 { status: "error", db: "unreachable" }. Railway reinicia automaticamente no 503. |
| I775 | **Variável ALLOWED_ORIGINS no Railway** | 🔴 Alta | ○ S | Sem isso, o frontend Vercel recebe erro CORS da API Railway | Adicionar nas env vars do Railway: ALLOWED_ORIGINS=https://pap-tan-seven.vercel.app,https://pap.sociedadetucci.com.br. O código já lê essa variável em allowedOrigins.ts. |
| I776 | **Governadora de Operações — função Mayumi** | 🟡 Média | ○ CONCEITO | Definir formalmente a função da Mayumi no Ecossistema Théo como âncora executiva | Mayumi = Governadora de Operações: valida o que entra no ciclo de execução (filtro), gestão financeira/patrimonial (Rapadura, imóveis), homologa atas da Assembleia que envolvem recursos reais. Contrapeso da arquitetura Yuri. Ferramentas: MYYM + Jasmim-Manga. |
| I777 | **Hub Territorial — consultoria local São Carlos/Rio Claro** | 🟢 Baixa | ○ CONCEITO | Modelo de negócio do casal para produtores locais após mudança | Mayumi reorganiza gestão/precificação do produtor; Yuri entrega identidade visual + IA de atendimento. Mini-agência de inteligência territorial, só com parceiros alinhados ao Cerrado/Mata Atlântica. Sem networking de massa. || I778 | **"Direito de Morrer do Token" — protocolo ético da crypto Arvore** | 🟡 Média | ○ CONCEITO | White paper ético da crypto Arvore precisa de capítulo sobre encerramento honroso | Token que sobrevive à morte da árvore = greenwashing de segunda ordem (ISA). O protocolo deve definir: (1) queima automática quando lastro falha, (2) reversão pro-rata para detentores, (3) reativação apenas com prova de replantio auditável. Théo = tradutor vigilante, nunca criador nem parceiro comercial. Co-branding máximo = campanha de conscientização. |
| I779 | **Crypto Arvore — projeto formal no Ecossistema Théo** | 🔴 Alta | ○ PROJETO | Formalizar crypto Arvore como projeto oficial com página no Jasmim e nó no Céu | MVP off-chain no ar (b2f8e8d): tabelas Neon + API + ArvoreTokenPage + ArvoreToken.sol. Próximos passos: deploy Polygon Amoy testnet, worker on-chain sync, Ecosia API bridge, página no Jasmim, nó no Céu. Custódio com CPF = bloqueio atual antes de Mainnet. |
| I780 | **Ecosia Bridge — contribuição digital sem blockchain própria** | 🟡 Média | ○ CONCEITO | Integrar Ecosia API como camada de verificação sem precisar de blockchain própria | Ecosia tem API pública (ecosia.org/api). Modelo: usuário faz pesquisa/ação → Ecosia planta árvore com GPS → Théo recebe callback → emite ARVR na ledger off-chain. Sem deploy de contrato. Zero custo. Prova de conceito em 1 dia. Ponto fraco: depende de Ecosia continuar com a API aberta. |

## Age — Rodadas 9-11 Mayumi + Yuri (2026-09-11)

| # | Feature | Prior. | Compl. | Impacto | Descrição técnica |
|---|---|---|---|---|---|
| I781 | **Dashboard inicial do profissional (panorama semana)** | 🔴 Alta | ◑ M | Profissional abre o Age e bate o olho: agenda, atraso, inadimplentes, desmarcados, pendências de aprovação | Tab "Início" com cards: consultas da semana, pacientes aguardando aprovação, inadimplentes, desmarcamentos recentes. Acoplar ao feed inteligente existente. |
| I782 | **IAs animadas comentam ações do usuário (padrão Jasmim)** | 🟡 Média | ○ M | Todas as IAs (SABIÁ, MYYM, ISA) devem falar sobre o que o usuário está fazendo, como o skininho do Jasmim | Fork do padrão de comentário contextual do Jasmim-Manga: estado idle → frase de boas-vindas; hover em botão → dica; erro → consolo; sucesso → celebração. Implementar em Age, Rapadura, Jasmim, PV. |
| I783 | **Carrinho de ideias direto da plataforma (Mayumi)** | 🟡 Média | ○ S | Mayumi não quer mandar email — quer sugerir diretamente no sistema | Ícone de "idea card" flutuante em todas as páginas da Mayumi (gestora + MYYM). Clica, escreve ideia, salva em tabela ideias_mayumi. Notificação para Yuri/Cláudio. |
| I784 | **Campo de triagem interativo e humorístico (paciente)** | 🟡 Média | ○ M | Formulário de doença/tratamento deve ser divertido no celular, sem jargão médico | Wizard com perguntas em linguagem cotidiana, humor leve, ilustrações emoji. Múltipla escolha > campo livre. Roda bem no celular. SABIÁ classifica o resultado em categoria clínica automaticamente. |
| I785 | **Relatório semanal "sonhos" da SABIÁ por email** | 🟡 Média | ○ S | SABIÁ estuda pacientes durante a semana → envia resumo toda segunda-feira para profissional e Yuri | Cron semanal (seg 7h): SABIÁ gera resumo do que aprendeu sobre cada paciente ativo — padrões, alertas, curiosidades. Envia para profissional + yurituccieterovic@gmail.com. |
| I786 | **Stripe automático por profissional (sem criar produto manual)** | 🔴 Alta | ○ M | Yuri não quer criar produto no Stripe para cada profissional — deve ser automático | Na criação do profissional: POST Stripe /products + /prices com valor configurado. Salvar stripe_product_id e stripe_price_id na tabela professionals. Subscription automática sem intervenção manual. |
| I787 | **Grupo de pacientes opcional no feed (comunidade)** | 🟢 Baixa | ○ M | Profissional pode habilitar um espaço onde pacientes trocam experiências | Toggle na config: "habilitar comunidade". Cria um canal no feed de notas com permissão para pacientes aprovados postar. Moderação pela profissional. LGPD: opt-in explícito do paciente. |

## Age — Sessões #675-#676 (2026-09-11)

| # | Feature | Prior. | Compl. | Impacto | Descrição técnica |
|---|---|---|---|---|---|
| I788 | **Age como plataforma genérica (cliente/profissional)** | 🔴 Alta | ○ M | Renomear "paciente/profissional" para "cliente/profissional" abre mercado: beleza, coaching, advogados, nutricionistas | Config por tipo de profissão: `tipo_cliente` e `tipo_profissional` configuráveis na criação da conta. Backend e UI usam esses labels. Um único código, múltiplos mercados. |
| I789 | **Assembleia no Jasmim — Mayumi lê deliberações** | 🟡 Média | ○ M | Mayumi não tem acesso aos emails da Assembleia — criar bridge IMAP → Jasmim | Worker que lê emails RESULTADO/PERFEITO do luddlocke@gmail.com e salva em tabela assembly_cache. Endpoint /api/jasmim/assembleia. UI no Jasmim com histórico filtrado por tag (#age, #jasmim, #rapadura). Segurança: same auth da Jasmim. |
| I790 | **SABIÁ conectada ao Stripe** | 🟡 Média | ○ M | SABIÁ pode fazer cobranças, enviar link de pagamento e confirmar recebimento diretamente no chat | Tool calls da SABIÁ: stripe_create_payment_link, stripe_check_status. Chat: "Sabiá, cria link de pagamento de R$150 para João" → gera e envia por email/WhatsApp. |
| I791 | **Esquilo CSS aprimorado — animação mais rica** | 🟢 Baixa | ○ S | Esquilo do Jasmim já funciona mas pode ser mais expressivo — cauda, olhos, sons suaves | Keyframes adicionais: cauda balança quando idle, olhos piscam em ritmo assimétrico, salta quando há nova mensagem, encolhe quando há erro. Referência: Yuri pediu inspiração na ISA da PAP. |
| I792 | **App mobile Age — Google Play + Apple Store** | 🟡 Média | ○ L | AGE tem PWA funcionando (iOS meta tags feitas) — publicar na loja aumenta alcance | PWA como TWA (Trusted Web Activity) no Android = publicação simples, sem custo de reescrita. iOS requer wrapper nativo mínimo. Google Play: US$25 (uma vez). Apple: US$99/ano. Fazer Android primeiro. |
| I793 | **Mercado Pago BR + Stripe Internacional** | 🔴 Alta | ○ M | Brasil tem alta adoção de MP (Pix, cartão local) — profissionais vão precisar dos dois | Yuri providencia conta MP. Integrar ao mesmo gateway middleware do Age: profissional escolhe MP ou Stripe por tipo de paciente. MP usa SDK MP Node.js. Stripe já tem chave nos secrets. |

## Age — Sessão #677 + Assembleia #678/679 (2026-09-11)

| # | Feature | Prior. | Compl. | Impacto | Descrição técnica |
|---|---|---|---|---|---|
| I794 | **SABIÁ como "tecnologia médica" — posicionamento comercial** | 🔴 Alta | ○ CONCEITO | Em vez de "agenda com IA", posicionar o AGE como "tecnologia médica com IA clínica integrada" — muda preço, público e narrativa | Materiais de venda: não é software de agenda, é parceira clínica digital. Foco em: memória persistente do paciente, análise de padrões clínicos, relatório semanal, alertas de risco. Preço sugerido: R$99-R$150/mês (vs R$59 broto). Público: clínicas, não freelancers. |

## Age — Assembleia #678: Chronos, Kairós e Semiótica (2026-09-11)

| # | Feature | Prior. | Compl. | Impacto | Descrição técnica |
|---|---|---|---|---|---|
| I795 | **SABIÁ Kairós — timing oportuno nas mensagens** | 🔴 Alta | ○ M | SABIÁ não deve falar na hora errada — deve identificar o momento certo (pré-cirurgia, pós-sessão, entrada no login) | Estado contextual por usuário: `estado_atual` (em consulta, livre, entrando, saindo). SABIÁ verifica estado antes de falar. Nunca interrompe foco. Fala quando há espaço de atenção. |
| I796 | **"Jardineiro do Tempo" — conceito para o Age** | 🟡 Média | ○ CONCEITO | Yuri propôs: "jardineiro do tempo" como tagline/propaganda — non como subtítulo mas como narrativa de marca | O profissional não gerencia agenda — cultiva tempo. A SABIÁ é o companheiro de cultivo. Usar em materiais de marketing, onboarding, e como frame para explicar a diferença Chronos/Kairós. |
| I797 | **SABIÁ poética — mensagens rimadas e eudaimônicas** | 🟡 Média | ○ S | SABIÁ deve "cantar" para as pessoas — comunicação marcante, rimada quando natural, sem ser infantil ou brega | Banco de frases poéticas por contexto: lembrete de consulta, aprovação de paciente, parabéns de sessão, silêncio produtivo. Tom: elegante, caloroso, #elegância como princípio. Inspiração: sabiá-laranjeira canta mas não grita. |
| I798 | **"O não nomeável como sintoma" — SABIÁ acolhe o inefável** | 🔴 Alta | ○ CONCEITO | Quando o paciente não consegue descrever o que sente, isso já é dado clínico. SABIÁ deve reconhecer e acolher sem forçar simbolização | Campo livre no formulário de triagem com análise semântica pelo SABIÁ. Resposta: não "o que você quis dizer?" mas acolhimento do vago. Nunca empurrar para mania. Manter equilíbrio. Feridas narcísicas tratadas com gentileza. |
| I799 | **Memória ativa vs. passiva na SABIÁ** | 🟡 Média | ○ CONCEITO | Distinção importante: SABIÁ não só guarda memória (passiva) — ela atua com base nela (ativa) | SABIÁ ativa: usa o histórico para antecipar necessidades, não só para responder perguntas. Ex: sabe que o paciente cancela quando está estressado → alerta o profissional antes. |


## Docs PAP — Ideias Novas (2026-09-12)

| # | Feature | Prior. | Compl. | Impacto | Descrição técnica |
|---|---|---|---|---|---|
| I800 | **Audit Log de /api/ai/*** | 🔴 Alta | ○ S | Rastrear todas as chamadas externas à API de agentes | Middleware em ai.ts que loga X-Api-Key parcial, endpoint, IP e timestamp em tabela ai_audit_log. Detecta abuso antes que vire custo. |
| I801 | **Connection Pool Tuning para Neon** | 🟡 Média | ○ S | Neon tem limite de conexões no free tier; pool mal configurado causa erros em pico | Configurar pg.Pool com max: 5 (Neon free: 10 conexões). Adicionar pool.on("error") para log. Considerar pgBouncer externo se ultrapassar. |
| I802 | **Migration System (drizzle-kit migrate)** | 🔴 Alta | ◑ M | push --force em produção pode apagar dados; migrations versionadas são seguras | Trocar drizzle-kit push por drizzle-kit generate + migrate. Criar pasta migrations/. Adicionar no Railway: step de migração no start command antes do node. |
| I803 | **Score Histórico por Semana** | 🟡 Média | ○ S | Permite mostrar evolução de XP semana a semana no heatmap | View ou query: SUM(node_code.length * 10) de exercise_attempts agrupado por semana ISO. Endpoint GET /api/progress/weekly-score. Gráfico de linha no menu. |
| I804 | **Paginação em /api/ai/nodes e /exercises** | 🟡 Média | ○ S | Com 57+ nós e centenas de exercícios, retornar tudo de uma vez é ineficiente | Query params: ?limit=50&offset=0. Resposta: { data: [...], total, limit, offset }. Não quebra clientes existentes (default limit alto). |
| I805 | **Health Check com DB Ping** | 🔴 Alta | ○ S | Railway usa /health para saber se o serviço está saudável; hoje retorna OK mesmo com DB morto | GET /health: faz SELECT 1 no pool. Se OK → 200 { status: "ok", db: "ok" }. Se falhar → 503 { status: "error", db: "unreachable" }. Railway reinicia automaticamente no 503. |
| I806 | **Variável ALLOWED_ORIGINS no Railway** | 🔴 Alta | ○ S | Sem isso, o frontend Vercel recebe erro CORS da API Railway | Adicionar nas env vars do Railway: ALLOWED_ORIGINS=https://pap-tan-seven.vercel.app,https://pap.sociedadetucci.com.br. O código já lê essa variável em allowedOrigins.ts. |
| I807 | **Relatório de uso por profissional (Lisange/Suzana)** | 🟡 Média | ○ S | Gerar PDF/texto com: slots ocupados, taxa de ocupação e histórico 30 dias para reuniões comerciais | Endpoint GET /api/age/:slug/relatorio?periodo=30d. Retorna JSON com total_slots, ocupados, taxa_pct, top_horarios. Mayumi usa na reunião com a profissional para mostrar o sistema em números reais. |
| I808 | **Cockpit comercial no painel Mayumi** | 🔴 Alta | ○ M | Mayumi precisa ver "health score" de cada profissional (risco de churn, baixo uso, sem login) para agir no momento certo | Adicionar ao painel /age/admin: colunas consultas_mes, dias_sem_login, pacientes_ativos. Calcular health: verde (>10 consultas, login recente), amarelo (5-10 ou 3-7 dias), vermelho (<5 ou 7+ dias). SABIÁ alerta proativamente quando vermelho. |
| I809 | **SABIÁ como primeiro contato comercial** | 🟡 Média | ○ M | Quando um profissional preenche formulário de interesse, SABIÁ responde antes de Mayumi | Webhook do formulário de interesse → SABIÁ gera resposta personalizada por email (tom poético, não de vendas). Inclui: "A Mayumi vai entrar em contato para uma conversa." Humaniza o funil antes do contato humano. |

## Docs PAP — Ideias Novas (2026-09-12)

| # | Feature | Prior. | Compl. | Impacto | Descrição técnica |
|---|---|---|---|---|---|
| I810 | **Audit Log de /api/ai/*** | 🔴 Alta | ○ S | Rastrear todas as chamadas externas à API de agentes | Middleware em ai.ts que loga X-Api-Key parcial, endpoint, IP e timestamp em tabela ai_audit_log. Detecta abuso antes que vire custo. |
| I811 | **Connection Pool Tuning para Neon** | 🟡 Média | ○ S | Neon tem limite de conexões no free tier; pool mal configurado causa erros em pico | Configurar pg.Pool com max: 5 (Neon free: 10 conexões). Adicionar pool.on("error") para log. Considerar pgBouncer externo se ultrapassar. |
| I812 | **Migration System (drizzle-kit migrate)** | 🔴 Alta | ◑ M | push --force em produção pode apagar dados; migrations versionadas são seguras | Trocar drizzle-kit push por drizzle-kit generate + migrate. Criar pasta migrations/. Adicionar no Railway: step de migração no start command antes do node. |
| I813 | **Score Histórico por Semana** | 🟡 Média | ○ S | Permite mostrar evolução de XP semana a semana no heatmap | View ou query: SUM(node_code.length * 10) de exercise_attempts agrupado por semana ISO. Endpoint GET /api/progress/weekly-score. Gráfico de linha no menu. |
| I814 | **Paginação em /api/ai/nodes e /exercises** | 🟡 Média | ○ S | Com 57+ nós e centenas de exercícios, retornar tudo de uma vez é ineficiente | Query params: ?limit=50&offset=0. Resposta: { data: [...], total, limit, offset }. Não quebra clientes existentes (default limit alto). |
| I815 | **Health Check com DB Ping** | 🔴 Alta | ○ S | Railway usa /health para saber se o serviço está saudável; hoje retorna OK mesmo com DB morto | GET /health: faz SELECT 1 no pool. Se OK → 200 { status: "ok", db: "ok" }. Se falhar → 503 { status: "error", db: "unreachable" }. Railway reinicia automaticamente no 503. |
| I816 | **Variável ALLOWED_ORIGINS no Railway** | 🔴 Alta | ○ S | Sem isso, o frontend Vercel recebe erro CORS da API Railway | Adicionar nas env vars do Railway: ALLOWED_ORIGINS=https://pap-tan-seven.vercel.app,https://pap.sociedadetucci.com.br. O código já lê essa variável em allowedOrigins.ts. |

## Docs PAP — Ideias Novas (2026-09-14)

| # | Feature | Prior. | Compl. | Impacto | Descrição técnica |
|---|---|---|---|---|---|
| I817 | **Audit Log de /api/ai/*** | 🔴 Alta | ○ S | Rastrear todas as chamadas externas à API de agentes | Middleware em ai.ts que loga X-Api-Key parcial, endpoint, IP e timestamp em tabela ai_audit_log. Detecta abuso antes que vire custo. |
| I818 | **Connection Pool Tuning para Neon** | 🟡 Média | ○ S | Neon tem limite de conexões no free tier; pool mal configurado causa erros em pico | Configurar pg.Pool com max: 5 (Neon free: 10 conexões). Adicionar pool.on("error") para log. Considerar pgBouncer externo se ultrapassar. |
| I819 | **Migration System (drizzle-kit migrate)** | 🔴 Alta | ◑ M | push --force em produção pode apagar dados; migrations versionadas são seguras | Trocar drizzle-kit push por drizzle-kit generate + migrate. Criar pasta migrations/. Adicionar no Railway: step de migração no start command antes do node. |
| I820 | **Score Histórico por Semana** | 🟡 Média | ○ S | Permite mostrar evolução de XP semana a semana no heatmap | View ou query: SUM(node_code.length * 10) de exercise_attempts agrupado por semana ISO. Endpoint GET /api/progress/weekly-score. Gráfico de linha no menu. |
| I821 | **Paginação em /api/ai/nodes e /exercises** | 🟡 Média | ○ S | Com 57+ nós e centenas de exercícios, retornar tudo de uma vez é ineficiente | Query params: ?limit=50&offset=0. Resposta: { data: [...], total, limit, offset }. Não quebra clientes existentes (default limit alto). |
| I822 | **Health Check com DB Ping** | 🔴 Alta | ○ S | Railway usa /health para saber se o serviço está saudável; hoje retorna OK mesmo com DB morto | GET /health: faz SELECT 1 no pool. Se OK → 200 { status: "ok", db: "ok" }. Se falhar → 503 { status: "error", db: "unreachable" }. Railway reinicia automaticamente no 503. |
| I823 | **Variável ALLOWED_ORIGINS no Railway** | 🔴 Alta | ○ S | Sem isso, o frontend Vercel recebe erro CORS da API Railway | Adicionar nas env vars do Railway: ALLOWED_ORIGINS=https://pap-tan-seven.vercel.app,https://pap.sociedadetucci.com.br. O código já lê essa variável em allowedOrigins.ts. |

## Docs PAP — Ideias Novas (2026-09-19)

| # | Feature | Prior. | Compl. | Impacto | Descrição técnica |
|---|---|---|---|---|---|
| I824 | **Audit Log de /api/ai/*** | 🔴 Alta | ○ S | Rastrear todas as chamadas externas à API de agentes | Middleware em ai.ts que loga X-Api-Key parcial, endpoint, IP e timestamp em tabela ai_audit_log. Detecta abuso antes que vire custo. |
| I825 | **Connection Pool Tuning para Neon** | 🟡 Média | ○ S | Neon tem limite de conexões no free tier; pool mal configurado causa erros em pico | Configurar pg.Pool com max: 5 (Neon free: 10 conexões). Adicionar pool.on("error") para log. Considerar pgBouncer externo se ultrapassar. |
| I826 | **Migration System (drizzle-kit migrate)** | 🔴 Alta | ◑ M | push --force em produção pode apagar dados; migrations versionadas são seguras | Trocar drizzle-kit push por drizzle-kit generate + migrate. Criar pasta migrations/. Adicionar no Railway: step de migração no start command antes do node. |
| I827 | **Score Histórico por Semana** | 🟡 Média | ○ S | Permite mostrar evolução de XP semana a semana no heatmap | View ou query: SUM(node_code.length * 10) de exercise_attempts agrupado por semana ISO. Endpoint GET /api/progress/weekly-score. Gráfico de linha no menu. |
| I828 | **Paginação em /api/ai/nodes e /exercises** | 🟡 Média | ○ S | Com 57+ nós e centenas de exercícios, retornar tudo de uma vez é ineficiente | Query params: ?limit=50&offset=0. Resposta: { data: [...], total, limit, offset }. Não quebra clientes existentes (default limit alto). |
| I829 | **Health Check com DB Ping** | 🔴 Alta | ○ S | Railway usa /health para saber se o serviço está saudável; hoje retorna OK mesmo com DB morto | GET /health: faz SELECT 1 no pool. Se OK → 200 { status: "ok", db: "ok" }. Se falhar → 503 { status: "error", db: "unreachable" }. Railway reinicia automaticamente no 503. |
| I830 | **Variável ALLOWED_ORIGINS no Railway** | 🔴 Alta | ○ S | Sem isso, o frontend Vercel recebe erro CORS da API Railway | Adicionar nas env vars do Railway: ALLOWED_ORIGINS=https://pap-tan-seven.vercel.app,https://pap.sociedadetucci.com.br. O código já lê essa variável em allowedOrigins.ts. |
## Assembleia #692 — Protocolo MIT: Inteligência → Aplicação → Mercado (2026-09-19)

### Age — Camada Comercial e Validação

| # | Feature | Prior. | Compl. | Impacto | Descrição técnica |
|---|---|---|---|---|---|
| I831 | **Entidade Opportunity no Age** | 🔴 Alta | ○ M | Cada profissional nova = uma oportunidade comercial rastreável com estado máquina completo | Tabela `age_opportunities`: id, slug_profissional, estado (IDEIA/HIPÓTESE/PROTÓTIPO/EXPERIMENTO/VALIDANDO/OFERTA/PROPOSTA/WON/LOST/DELIVERING/DELIVERED/RECURRING/ARCHIVED), criado_em, atualizado_em, problema, hipótese, oferta, resultado, aprendizado. Relaciona-se com age_professionals. |
| I832 | **Reality Board no painel Mayumi** | 🔴 Alta | ○ M | Dashboard que mostra a passagem IDEIA→REALIDADE: contatos, conversas, leads, propostas, vendas, receita | Adicionar aba "Comercial" no painel /age/admin: cards com contadores (profissionais contatadas, demos feitas, propostas, ativas pagas, receita mensal). Dados de age_opportunities + age_mensalidades. |
| I833 | **Backward Design template** | 🟡 Média | ○ S | Antes de programar qualquer nova feature, responder: resultado desejado → como medir → quem → IA onde ajuda → o que automatizar | Template markdown em docs/age/06-backward-design-template.md. Quando Mayumi ou Yuri pedir nova feature, Cláudio preenche o template antes de codificar. |
| I834 | **Loop de Aprendizagem formal no Age** | 🟡 Média | ○ S | Cada rodada de Assembleia ou reunião com profissional gera um ciclo registrável: OBSERVAR→INTERPRETAR→HIPOTETIZAR→EXPERIMENTAR→MEDIR→APRENDER | Endpoint POST /api/age/learnings { fonte, observacao, hipotese, experimento, resultado, aprendizado }. Feed de aprendizados no painel Mayumi. Assembleia consulta esses dados. |
| I835 | **Anti-monumento check** | 🟡 Média | ○ CONCEITO | Cada feature nova responde obrigatoriamente: qual é seu tipo? (PRODUTO/SERVIÇO/INFRAESTRUTURA/PESQUISA/DEMONSTRAÇÃO) | Coluna `tipo_artefato` em IDEIAS.md. Regra: nenhum projeto cresce sem evidência proporcional de valor. Implementar como checklist no template de ideia nova. |

### Ecossistema — Camada de Mercado Transversal

| # | Feature | Prior. | Compl. | Impacto | Descrição técnica |
|---|---|---|---|---|---|
| I836 | **Opportunity entity PAP** | 🟡 Média | ○ M | Aluno do PAP como oportunidade rastreável: INSCRITO_FUVEST → APROVADO → RETENDO → PAGANTE | Estados em tabela pap_opportunities. Link com users.tier. Mostra taxa de conversão FUVEST real (hipótese validada). |
| I837 | **Processo comercial do Ecossistema** | 🔴 Alta | ○ CONCEITO | Para cada capacidade do ecossistema responder: PARA QUEM? RESOLVE O QUÊ? COMO MOSTRAMOS? COMO ENTREGAMOS? COMO COBRAMOS? | Documento docs/ecosistema/mapa-de-capacidades.md. Preencher por sistema: PAP / Age / Rapadura / PV / Consultoria / Audiovisual / REI. Estado inicial: DESCONHECIDO para maioria. |
| I838 | **Reality Board transversal** | 🟡 Média | ○ L | Dashboard que mostra passagem IDEIA→REALIDADE em TODOS os projetos simultaneamente | Rota /realidade no painel admin. Tabela global `st_opportunities` com projeto_origem. Cards por projeto com estado atual. Prioridade após Reality Board do Age. |
| I839 | **docs/age/ structure (anti-Lost-in-Middle)** | 🔴 Alta | ○ S | 13 documentos canônicos do Age para evitar decisões silenciosamente substituídas | Criar: 00-manifest.md, 01-current-truth.md, 02-architecture.md, 03-decisions.md, 04-reuse-map.md, 05-commercial-model.md, 06-opportunity-model.md, 07-offer-model.md, 08-experiment-model.md, 09-metrics.md, 10-integration-map.md, 11-open-questions.md, 12-lost-in-middle-recovery.md |
| I840 | **Human-AI boundary policy formal** | 🟡 Média | ○ S | Documentar explicitamente o que IA decide vs humano decide no Age | Em 00-manifest.md: IA PODE (pesquisar, organizar, sugerir, alertar, automatizar tarefas autorizadas). IA NÃO DECIDE (compromisso contratual, alteração financeira, representação externa). Mayumi = ponto de homologação humano. |
