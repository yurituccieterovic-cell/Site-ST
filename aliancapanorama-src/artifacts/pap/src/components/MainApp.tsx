...
function MainAppInner({ queryClient }: { queryClient: ReturnType<typeof useQueryClient> }) {
  ...
  return (
    ...
    >
      ...
      <SubscriptionDowngradeBanner onOpenPlans={() => setPlansOpen(true)} />
    </div>
  );
}

/* ─── Subscription downgrade banner ──────────────────────────────────────── */
function SubscriptionDowngradeBanner({ onOpenPlans }: { onOpenPlans: () => void }) {
  const { user, refetch } = useAuth();
  const status = user?.subscriptionStatus ?? null;
  const downgradedAt = user?.lastDowngradeAt ?? null;
  const downgradeStatuses = new Set(["cancelled", "expired", "suspended", "past_due"]);
  const visible = !!user && !!status && downgradeStatuses.has(status) && !!downgradedAt;
  ...
  const STATUS_TEXT: Record<string, string> = {
    ...
    past_due: "Pagamento da sua assinatura falhou",
  };
  const headline = STATUS_TEXT[status!] ?? "Sua assinatura foi encerrada";

  const handleDismiss = async () => {
    setDismissing(true);
    try {
      await fetch("/api/auth/dismiss-downgrade-notice", {
        method: "POST",
        credentials: "include",
      });
      refetch();
  ...
  return (
    <motion.div
      ...
      exit={{ y: -40, opacity: 0 }}
      className="absolute top-2 left-1/2 -translate-x-1/2 z-[90] max-w-[92%] w-[420px] rounded-xl border border-orange-400/40 px-3 py-2 flex items-center gap-2 shadow-lg"
      style={{ background: "rgba(40,15,5,0.92)", backdropFilter: "blur(8px)" }}
    >
      <Bell className="w-3.5 h-3.5 text-orange-300 shrink-0" />
      <div className="flex-1 min-w-0">
        <p className="text-[10px] font-bold text-orange-100 leading-tight">{headline}</p>
        <p className="text-[9px] text-orange-200/70 leading-tight mt-0.5">
          Você voltou para o Aluno I. Renove para recuperar o acesso completo.
        </p>
      </div>
      <button
        onClick={onOpenPlans}
        className="text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-md bg-orange-300/90 text-black hover:bg-orange-200 transition-colors shrink-0"
      >
        Renovar
      </button>
      <button
        ...
        className="p-1 rounded-md hover:bg-white/10 transition-colors shrink-0 disabled:opacity-50"
      >
...
function NodeModal({ code, onClose, onNodeOpen, onAchievementEarned, onExercise, userTier }: {
  ...
  const SITE = "https://pap.sociedadetucci.com.br";
